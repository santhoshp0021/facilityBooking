...
const FacilityWiseBooking = () => {
  ...
  const handleBook = async (facilityName, type, idx) => {
    console.log('hi',selectedDate,idx, facilityName, type, user.userId );
    const payload = {
      date: selectedDate,
      slot: idx,
    ...
    try {
      ...
      await response.json();
      alert(` ${facilityName} successfully booked for ${selectedDate} - ${idx+1}`);
                ...
                {periods.map((p, idx) => {
                  ...
                  return (
                    <div key={idx} style={{ padding: '6px 10px', background: match ? '#f8d7da' : '#e0f2f1', borderRadius: 6, minWidth: 120 }}>
                      ...
                      {!match && (
                        <button onClick={() => handleBook(name, type,idx)} style={{ marginTop: 4, padding: '2px 6px', fontSize: 12 }}>Book</button>
                      )}
                    </div>
                  );
                })}
